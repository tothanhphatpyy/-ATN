  ...
  const handleFollowOA = (path) => {
    if (!userInfo?.phone) {
      confirmModal.show({
        ...
        title: "Đăng ký mới",
        content: `Quan tâm ${getConfig((c) => c.app.title)} để có thể sử dụng toàn bộ các tính năng của ứng dụng!`,
        acceptButton: {
          text: "Quan tâm ngay!",
          onClick: () => {
            handleFollow(path);
  ...
  const handleFollowPhone = (path) => {
    if (!userInfo?.phone) {
      confirmModal.show({
        type: ConfirmType.Follow,
        title: "Đăng ký mới",
        content: `Cấp quyền truy cập số điện thoại cho ${getConfig((c) => c.app.title)} để có thể sử dụng các tính năng đặt hàng, đăng ký thành viên..`,
        acceptButton: {
          text: "Truy cập ngay!",
          onClick: () => {
